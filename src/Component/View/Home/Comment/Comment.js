...
const Comment = (foodDetail) => {
  ...
  const fetchCommnet = async () => {
    await axiosConfig
      .get(
        `/user/foodReview/findfoodReviewByFoodId/${foodDetail.foodDetail.foodId}`
      )
      .then((response) => {
  ...
  const fetchUser = async () => {
    try {
      await axiosConfig.get(`user/${userId}`).then((response) => {
        ...
        <div key={comment.id} className="comment-item">
          ...
          <div className="reply">
            ...
            <div className="reply-message">
              Cảm ơn quý khách đã tin yêu và ủng hộ TASTY, chúc quý khách có
              những trải nghiệm tuyệt vời cùng TASTY!
            </div>
          </div>
          <br />
        </div>
