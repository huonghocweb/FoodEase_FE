...
const AddFoodVariation = ({ onClose, item }) => {
  ...
  const fetchFoodVariation = async () => {
    try {
      const response = await axiosConfig.get(
        `/user/foodvariation/findFoodVariationByFoodId/${item.foodId}`
      );
  ...
  const handleAdd = async (data) => {
    try {
      ...
      setAlert({ type: "success", message: customTranslate("Added Success!") });
      fetchFoodVariation();
    } catch (error) {
      setAlert({
        type: "error",
        message: customTranslate("Added Failed,Size already exists!"),
      });
  ...
  const handleUpdate = async (data) => {
    try {
      ...
      await axiosConfig.put(
        `/user/foodvariation/updateFoodVariation/${foodVariationId}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      setAlert({
        type: "success",
        message: customTranslate("Update Success!"),
      });
      fetchFoodVariation();
    } catch (error) {
      setAlert({ type: "error", message: customTranslate("Update Failed!") });
      console.error("Error:", error);
    }
  };
  ...
  const deleteFoodVariation = async (id) => {
    try {
      ...
      setAlert({
        type: "success",
        message: customTranslate("Delete Success!"),
      });
    } catch (error) {
      setAlert({ type: "error", message: customTranslate("Delete Failed!") });
    }
  };
  ...
  const handleCreateFoodVariationToppings = async () => {
    try {
      ...
      setAlert({
        type: "success",
        message: customTranslate("Add topping Success!"),
      });
    } catch (error) {
      console.error(error);
      setAlert({
        type: "error",
        message: customTranslate("Add topping error!"),
      });
    }
  };
  useEffect(() => {
  ...
  return (
    <div className="modalt">
      <div className="modal-contentt">
        <span className="closet" onClick={onClose}>
          &times;
        </span>
        <form onSubmit={handleSubmit(edit1 ? handleUpdate : handleAdd)}>
          <h1>{customTranslate("")}Food variation</h1>
          <div className="foodvariation-input">
            <div>
              <input
                className="input-quantity"
                type="number"
                placeholder={customTranslate("Quantity Stock")}
                {...register("quantityStock", {
                  required: customTranslate("Required field"),
                  min: {
                    value: 0,
                    message: customTranslate("Quantity must be positive"),
                  },
                })}
              />
              {errors.quantityStock && (
                <p className="error">{errors.quantityStock.message}</p>
              )}
            </div>
            ...
            <div>
              <select
                {...register("FoodSizeId", {
                  required: customTranslate("Please select a size"),
                })}
              >
                <option value="">{customTranslate("Select a size")}</option>
                {sizes.map((size) => (
                  <option key={size.id} value={size.id}>
                    {customTranslate(`${size.name}`)}
                  </option>
                ))}
              </select>
              {errors.FoodSizeId && (
                <p className="error">{errors.FoodSizeId.message}</p>
              )}
            </div>
            ...
            <button type="submit">
              {edit1 ? customTranslate("Update") : customTranslate("Create")}
            </button>
        ...
        {foodVariation.map((varItem, index) => (
          <div key={index} className="custom-modal-bodyt">
            <img
              onClick={() => handleImgClick(varItem.foodVariationId)}
            ...
            <div className="order-detailst">
              <h4>
                ...
                {customTranslate(`${varItem.foodSize.foodSizeName}`)}
              </h4>
              <h4>
                {customTranslate("Quantity")}: {varItem.quantityStock}
              </h4>
              <h4>
                {customTranslate("Discount")}: {varItem.food.discount}%
              </h4>
            </div>

            <button onClick={() => edit(varItem)} className="button-edit">
              {customTranslate("Edit")}
            </button>
            <button
              onClick={() => deleteFoodVariation(varItem.foodVariationId)}
            ...
            <div className="toppings">
              {toppings.map((topping) => (
                <div key={topping.id}>
                  ...
                  />
                  <span> {customTranslate(`${topping.name}`)}</span>
                </div>
              ))}
            </div>

            <button type="submit">
              {customTranslate("Add foodVariation")}
            </button>
          </form>
        </div>
      )}
