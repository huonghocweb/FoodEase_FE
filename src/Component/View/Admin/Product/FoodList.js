...
const FoodList = () => {
  ...
  const fetchCategories = async () => {
    await axiosConfig.get(`/categories/findAll`).then((response) => {
      setCategories(response.data);
    });
  };
  const fetchFood = async () => {
    await axiosConfig
      .get(`/user/food/findMain?page=${page}`)
      .then((response) => {
  ...
  const handleAddCategory = async () => {
    try {
      await axiosConfig.post(
        `/categories/addFoodCategory?categoryName=${inputCategory}`
      );
      setInputCategory("");
    } catch (error) {
            ...
            <div className="tm-bg-primary-dark tm-block tm-block-products col-sm-12">
              <div className="tm-product-table-container">
                <table className="table table-hover tm-table-small tm-product-table">
                  <thead>
                    <tr>
                      <th scope="col">{customTranslate("FOOD NO.")}</th>
                      <th scope="col">{customTranslate("FOOD NAME")}</th>
                      <th scope="col">{customTranslate("BASE PRICE")}</th>
                      <th scope="">{customTranslate("IMAGE")}</th>
                      <th scope="col">{customTranslate("CREATED AT")}</th>
                      <th scope="col">{customTranslate("DISCOUNT")}</th>
                      <th scope="col">{customTranslate("CATEGORY")}</th>
                      <th>{customTranslate("FUNCTION")}</th>
                  ...
                  <tbody>
                    {Food.map((item, index) => (
                      <tr key={item.foodId}>
                        ...
                        <td>{item.discount}%</td>
                        <td>
                          {customTranslate(`${item.category.cartegoryName}`)}
                        </td>
                        <td>
                          <i
                            onClick={() => edit(item)}
                  ...
                  >
                    {customTranslate("Add Food")}
                  </Link>
                  <button className="btn btn-primary btn-block text-uppercase">
                    {customTranslate("Delete Food")}
                  </button>
                </table>
                {isModalOpen && (
          ...
          <div className="col-sm-12 col-md-12 col-lg-4 col-xl-4 tm-block-col">
            <div className="tm-bg-primary-dark tm-block tm-block-product-categories">
              <h2 className="tm-block-title">
                {customTranslate("Product Categories")}
              </h2>
              <div className="tm-product-table-container">
                <table className="table tm-table-small tm-product-table">
                      ...
                      <tr>
                        ...
                        <td className="text-center">
                          <a href="#" className="tm-product-delete-link">
                            <i
                              onClick={() =>
                                deleteCategory(item.foodCategoriesID)
                              }
                              className="far fa-trash-alt tm-product-delete-icon"
                            ></i>
                          </a>
                  ...
                  <button
                    ...
                    className="btn btn-primary btn-block text-uppercase mb-3"
                  >
                    {customTranslate("Add new category")}
                  </button>
                </table>
                <input
                  value={inputCategory}
